...
const corsHandler = cors({
  origin: [/olybars\.com$/, /firebaseapp\.com$/, /localhost/],
});
...
export const syncUserProfile = onDocumentWritten(
  "users/{userId}",
  async (event) => {
    ...
    if (roleChanged || adminChanged) {
      try {
        ...
        logger.info(`[Auth] Synced custom claims for ${userId}`, {
          role: newUser.role,
        });
      } catch (error) {
        logger.error(
          `[Auth] Failed to sync custom claims for ${userId}`,
          error,
        );
      }
    }
    ...
    const publicProfile = {
      ...
      league_stats: {
        ...
        rank:
          newUser.league_stats?.rank ||
          existingData?.league_stats?.rank ||
          "Unranked",
      },
...
export { generateHolidayBriefing } from "./triggers/scheduled/generateHolidayBriefing";

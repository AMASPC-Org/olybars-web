...
export const claimVenueFlow = onCall(
  ...
  async (request) => {
    ...
    if (!request.auth)
      throw new Error("Unauthorized: You must be signed in to claim a venue.");
    ...
    if (!googleData)
      throw new Error("Could not fetch details for this venue from Google.");
    ...
    if (!existingSnap.empty) {
      throw new Error(
        "This venue has already been claimed or added to the system.",
      );
    }
    ...
    const venueData = {
      ...
      venueType: (googleData.types.includes("bar")
        ? "Bar"
        : "Restaurant") as any, // Simple mapping
      status: {
        vibe: "chill",
        volume: 50,

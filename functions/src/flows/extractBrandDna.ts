...
export const extractBrandDnaFlow = onCall(
  ...
  async (request) => {
    ...
    if (uploadedImages && uploadedImages.length > 0) {
      console.log(
        `[BrandAudit] Tier 1: Analyzing ${uploadedImages.length} uploaded images.`,
      );
    ...
    if (googlePhotos.length > 0) {
      console.log(
        `[BrandAudit] Tier 2: Analyzing ${googlePhotos.length} Google photos.`,
      );
...
async function analyzeVisuals(
  ...
  const result = await ai.generate({
    ...
    prompt: [
      ...
      ...imageUrls
        .slice(0, 5)
        .map((url) => ({ media: { url, contentType: "image/jpeg" } })),
    ],

...
export const generateSocialFlyerFlow = onCall(
  ...
  async (request) => {
    ...
    const dimensions = getSocialDimensions(
      input.spec.platform as any,
      input.spec.type,
    );
    if (!dimensions)
      throw new Error(
        `Unknown format: ${input.spec.platform} / ${input.spec.type}`,
      );
    ...
    if (!media || !media.data)
      throw new Error("AI generated no image media data.");

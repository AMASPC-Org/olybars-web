...
export const SCHMIDT_SKILLS: Record<string, SchmidtSkill> = {
  schedule_flash_deal: {
...
5. PRICE: Must be a clear dollar amount or percentage off.
...
TRAFFIC: If the slot is "BUSY", warn the user they will rotate with up to 2 other bars on the homepage.`,
    params: [
      ...
      {
        name: "startTimeISO",
        description: "ISO string of the start time",
        required: true,
      },
      { name: "duration", description: "Duration in minutes", required: true },
      {
        name: "staffBriefingConfirmed",
        description: "Boolean confirming staff is aware",
        required: true,
      },
      { name: "price", description: "Optional price string", required: false },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "schedule_flash_deal", "params": {"summary": "{{summary}}", "details": "{{details}}", "startTimeISO": "{{startTimeISO}}", "duration": "{{duration}}", "staffBriefingConfirmed": {{staffBriefingConfirmed}}, "price": "{{price}}"}}',
  },
  update_hours: {
    id: "update_hours",
    name: "Update Hours",
...
Confirm the new hours string (e.g., "Mon-Fri 4pm-10pm, Sat-Sun 12pm-11pm").`,
    params: [
      {
        name: "hours",
        description: "The formatted hours string",
        required: true,
      },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "update_hours", "params": {"hours": "{{hours}}"}}',
  },
  update_happy_hour: {
...
Confirm both before generating action.`,
    params: [
      {
        name: "schedule",
        description: "Days and times of happy hour",
        required: true,
      },
      { name: "specials", description: "Summary of deals", required: true },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "update_happy_hour", "params": {"schedule": "{{schedule}}", "specials": "{{specials}}"}}',
  },
  add_event: {
    id: "add_event",
    name: "Add Event",
...
COMPLIANCE: Ensure the event does not reward points for alcohol purchase and avoids "Anti-Volume" marketing.`,
    ...
    actionTemplate:
      '[ACTION]: {"skill": "add_event", "params": {"type": "{{type}}", "time": "{{time}}", "description": "{{description}}"}}',
  },
  update_profile: {
    id: "update_profile",
    name: "Update Profile",
...
Confirm the specific changes before generating the action.`,
    params: [
      ...
      {
        name: "description",
        description: "Venue description/bio",
        required: false,
      },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "update_profile", "params": {"website": "{{website}}", "instagram": "{{instagram}}", "facebook": "{{facebook}}", "description": "{{description}}"}}',
  },
  draft_social_post: {
...
4. Present the draft for the user to "Save to Drafts".`,
    ...
    actionTemplate:
      '[ACTION]: {"skill": "draft_social_post", "params": {"topic": "{{topic}}", "copy": "{{copy}}", "platform": "{{platform}}"}}',
  },
  ideate_event: {
    id: "ideate_event",
    name: "Event Ideation",
...
4. If the user likes one, generate a draft for them.`,
    ...
    actionTemplate:
      '[ACTION]: {"skill": "ideate_event", "params": {"concepts": "{{concepts}}"}}',
  },
  add_menu_item: {
    id: "add_menu_item",
    name: "Add Menu Item",
...
Confirm the details before saving.`,
    params: [
      ...
      {
        name: "description",
        description: "Details/Ingredients",
        required: true,
      },
      { name: "price", description: "Price string", required: false },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "add_menu_item", "params": {"category": "{{category}}", "name": "{{name}}", "description": "{{description}}", "price": "{{price}}"}}',
  },
  promote_menu_item: {
...
4. Present for "Save to Drafts".`,
    params: [
      {
        name: "item_name",
        description: "The name of the item being promoted",
        required: true,
      },
      {
        name: "copy",
    ...
    actionTemplate:
      '[ACTION]: {"skill": "promote_menu_item", "params": {"item_name": "{{item_name}}", "copy": "{{copy}}"}}',
  },
  emergency_closure: {
    id: "emergency_closure",
    name: "Emergency Closure",
    description:
      "Temporarily close the venue for the rest of the day or a specific duration.",
    category: "PROFILE",
    protocol: `
...
4. Future events scheduled after this window will remain visible.`,
    params: [
      ...
      {
        name: "duration",
        description: "How long (e.g. 1 day, rest of today)",
        required: true,
      },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "emergency_closure", "params": {"reason": "{{reason}}", "duration": "{{duration}}"}}',
  },
  update_order_url: {
    id: "update_order_url",
...
3. Update the venue profile link.`,
    ...
    actionTemplate:
      '[ACTION]: {"skill": "update_order_url", "params": {"url": "{{url}}"}}',
  },
  update_capacity: {
    id: "update_capacity",
...
2. Confirm the number before updating.`,
    params: [
      {
        name: "capacity",
        description: "The maximum occupancy number",
        required: true,
      },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "update_capacity", "params": {"capacity": {{capacity}}}}',
  },
  confirm_social_draft: {
    id: "confirm_social_draft",
    name: "Confirm Social Draft",
    description:
      "Approve or reject a drafted social media post for publishing.",
    category: "CONTENT_ENGINE",
    protocol: `
1. Present the draft details (caption, platform).
2. Ask for a final "Confirm" or "Revise".
3. Once confirmed, Schmidt schedules the post for immediate distribution.`,
    params: [
      {
        name: "draftId",
        description: "ID of the social draft",
        required: true,
      },
      { name: "status", description: "APPROVED or REJECTED", required: true },
    ],
    actionTemplate:
      '[ACTION]: {"skill": "confirm_social_draft", "params": {"draftId": "{{draftId}}", "status": "{{status}}"}}',
  },
};

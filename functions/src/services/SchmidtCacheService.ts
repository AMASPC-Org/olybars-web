...
/**
 * SchmidtCacheService: Phase 2/3 FinOps Maturity
 * Manages Vertex AI Context Caching for the static parts of Schmidt's brain.
 */
export class SchmidtCacheService {
  ...
  /**
   * Retrieves or creates a Vertex AI Content Cache for Schmidt's static instructions.
   */
  static async getOrSetStaticCache(
    ...
    if (approxTokens < this.TOKEN_THRESHOLD) {
      console.log(
        `[SCHMIDT] Lore context (${approxTokens} tokens) below threshold. Skipping cache.`,
      );
      return null;
    }
    ...
    try {
      console.log(
        `[SCHMIDT] Initializing context cache for Schmidt's KB (${approxTokens} tokens)...`,
      );
      ...
      console.log(`[SCHMIDT] Context cache created: ${this.cacheName}`);
      return this.cacheName;
    } catch (error: any) {
      console.warn("[SCHMIDT] Context Caching failed:", error.message);
      return null;
    }
  }

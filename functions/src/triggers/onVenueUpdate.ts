...
export const onVenueUpdate = onDocumentWritten("venues/{venueId}", async (event) => {
  // 1. Validation
  ...
  for (const source of afterData.scraper_config) {
    ...
    const oldSource = beforeData?.scraper_config?.find(s => s.id === source.id);
  ...
  console.log(`[onVenueUpdate] Venue ${venueId} has ${pendingSources.length} pending sources. Dispatching.`);
  ...
  console.log(`[onVenueUpdate] Dispatched ${publishPromises.length} immediate jobs.`);
});

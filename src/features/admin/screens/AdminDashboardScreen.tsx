...
import { HolidayIntelligenceTab } from "../components/HolidayIntelligenceTab";
...
export const AdminDashboardScreen: React.FC<AdminDashboardScreenProps> = ({
  ...
  const handleSaveCalibration = async (coords: {
    lat: number;
    lng: number;
  }) => {
  ...
  const filteredLeagueUsers = leagueUsers
    ...
    .sort(
      (a, b) => (b.stats?.seasonPoints || 0) - (a.stats?.seasonPoints || 0),
    );
  ...
  const filteredVenues = venues
    .filter(
      (v) =>
        ...
        v.scraper_config?.some((s) =>
          s.url.toLowerCase().includes(searchTerm.toLowerCase()),
        ),
    )
    .filter((v) => {
      if (venueFilter === "visible")
        return v.isVisible !== false && v.isActive !== false;
      if (venueFilter === "ghost")
        return v.isVisible === false && v.isActive !== false;
  ...
  const onToggleVenueMembership = async (
    venueId: string,
    ...
    queryClient.setQueryData(["venues-brief"], (old: Venue[] | undefined) => {
      return old?.map((v) =>
        v.id === venueId ? { ...v, isPaidLeagueMember: !currentStatus } : v,
      );
    });
  ...
  const onToggleVisibility = async (
    ...
    queryClient.setQueryData(["venues-brief"], (old: Venue[] | undefined) => {
      return old?.map((v) =>
        v.id === venueId ? { ...v, isVisible: !currentStatus } : v,
      );
    });
  ...
  const onToggleGameVibe = async (venueId: string, currentStatus: boolean) => {
    ...
    queryClient.setQueryData(["venues-brief"], (old: Venue[] | undefined) => {
      return old?.map((v) =>
        v.id === venueId
          ? { ...v, hasGameVibeCheckEnabled: !currentStatus }
          : v,
      );
    });
    ...
    try {
      await updateVenueDetails(venueId, {
        hasGameVibeCheckEnabled: !currentStatus,
      });
      queryClient.invalidateQueries({ queryKey: ["venues-brief"] });
    } catch (error) {
  ...
  const handleApprovePhoto = async (venueId: string, photoId: string) => {
    ...
    const updatedPhotos = venue.photos.map((p) =>
      p.id === photoId
        ? ({
            ...p,
            marketingStatus: "pending-venue",
            superAdminApprovedBy: userProfile.uid,
          } as any)
        : p,
    );
  ...
  const stats = [
    ...
    {
      label: "Total Points",
      value: systemStats.totalPoints.toLocaleString(),
      icon: Database,
    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-6 font-sans">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-black font-league uppercase tracking-tight">
            System Dashboard
          </h1>
        </div>
        <div className="text-right">
          <p className="text-sm font-bold text-white uppercase">
            {userProfile.handle || "Ryan Admin"}
          </p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="bg-slate-900/50 border border-white/5 p-4 rounded-2xl shadow-xl"
          >
      ...
      <div className="bg-slate-900 border border-white/5 rounded-2xl p-6 min-h-[300px] shadow-2xl">
        {activeTab === "overview" && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-black font-league uppercase">
                Live Activity Loop
              </h2>
                ...
                recentActivity.map((log) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center ${log.verificationMethod === "qr" ? "bg-yellow-500/20 text-yellow-500" : "bg-slate-700/50 text-slate-400"}`}
                      >
                    ...
                    <div className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        {log.verificationMethod === "qr" && (
                          <span className="text-[9px] font-black bg-yellow-500 text-black px-1.5 rounded uppercase">
                            Verified
                          </span>
                        )}
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                      venueFilter === f
                        ...
                        : "text-slate-500 hover:text-white bg-black/40"
                    }`}
                  >
                    {f}
                  </button>
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <div
                          ...
                          title="Active Scraper Sources"
                        >
                          {venue.scraper_config?.some(
                            (s) => s.status === "error",
                          ) ? (
                            <AlertTriangle className="w-4 h-4 text-red-500" />
                          ) : (
                            <span
                              className={`text-[10px] font-black font-mono ${venue.is_scraping_enabled && (venue.scraper_config?.length || 0) > 0 ? "text-green-400" : "text-slate-700"}`}
                            >
                              {venue.scraper_config?.filter((s) => s.isEnabled)
                                .length || 0}
                              /{venue.scraper_config?.length || 0}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="p-3 text-right">
                        <button
                          onClick={() =>
                            onToggleActivity(venue.id, venue.isActive !== false)
                          }
                          className={`p-2 rounded-lg transition-all ${
                          ...
                          title={
                            venue.isActive !== false ? "Active" : "Archived"
                          }
                        >
                          {venue.isActive !== false ? (
                            <Power size={16} />
                  ...
                  {leagueUsers
                    ...
                    .map((user) => (
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          <div
                            className={`w-2 h-2 rounded-full mx-auto ${user.email ? "bg-green-500" : "bg-slate-700"}`}
                          />
                        </td>
                      </tr>
                    ))}
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredLeagueUsers.map((user, idx) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <div
                          className={`w-2 h-2 rounded-full mx-auto ${user.role !== "guest" ? "bg-green-500" : "bg-slate-700"}`}
                        />
                      </td>
                    </tr>
                  ))}
          ...
          <div className="space-y-6">
            ...
            {pendingPhotos.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 bg-black/20 rounded-2xl border border-dashed border-white/5">
                ...
                <p className="text-[10px] text-slate-600 uppercase mt-1">
                  No new photos need verification.
                </p>
              </div>
            ) : (
        ...
        {activeTab === "calendar" && <HolidayIntelligenceTab />}
      </div>
      ...
      <div className="mt-8 p-4 bg-amber-400/10 border border-amber-400/20 rounded-2xl flex gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-300 font-medium">
            All admin actions are logged to Secret Manager and Cloud Audit.
            Proceed with League Integrity in mind.
          </p>
        </div>
      </div>
    </div>

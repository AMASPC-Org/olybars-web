...
import { Venue, UserProfile, AppEvent, ScheduledDeal } from "../../../types";
...
export const VenueProfileScreen: React.FC = () => {
  ...
  // [NEW] Upcoming Deal (Fetch from Subcollection or Registry)
  const [upcomingDeal, setUpcomingDeal] = useState<ScheduledDeal | null>(null);

  useEffect(() => {
    if (!id) return;
    const fetchUpcomingDeal = async () => {
      try {
        // Simple query: Deals ending in the future
        // We sort by startTime client-side to avoid complex indexes
        const now = Date.now();
        const q = query(
          collection(db, "venues", id, "scheduledDeals"),
          where("endTime", ">", now),
          limit(5),
        );
        const snapshot = await getDocs(q);
        const deals = snapshot.docs
          .map((d) => d.data() as ScheduledDeal)
          .filter((d) => d.startTime > now) // Must start in future (otherwise it's active)
          .sort((a, b) => a.startTime - b.startTime);

        if (deals.length > 0) {
          setUpcomingDeal(deals[0]);
        } else {
          setUpcomingDeal(null);
        }
      } catch (err) {
  ...
  useEffect(() => {
    const loadUpcomingEvents = async () => {
      ...
      try {
        const fetched = await EventService.fetchEvents({
          venueId: id,
          status: "approved",
        });
  ...
  const flightStats = React.useMemo(() => {
    ...
    const totalAbv = flightItems.reduce(
      (acc, item) => acc + (item.stats?.abv || 0),
      0,
    );
    return {
  ...
  const getLDSchema = () => {
    ...
    const baseSchema: any = {
      ...
      address: {
        ...
        addressLocality: venue.address?.includes("Lacey")
          ? "Lacey"
          : venue.address?.includes("Tumwater")
            ? "Tumwater"
            : "Olympia",
        addressRegion: "WA",
        addressCountry: "US",
      ...
      servesCuisine:
        venue.venueType === "restaurant_bar" ? venue.vibe : undefined,
      priceRange: "$$",
  ...
  return (
    <div className="bg-background min-h-screen pb-32 font-body text-slate-100 animate-in fade-in duration-500">
      ...
      <SEO
        ...
        description={
          venue.insiderVibe ||
          `Explore ${venue.name} in Thurston County. Live vibes, happy hours, and league play.`
        }
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        {venue.isLocalMaker &&
          venue.makerType !== "Distillery" &&
          venue.makerType !== "Brewery" && (
            <div className="absolute top-24 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-xl border border-primary/50 px-6 py-2 rounded-full shadow-[0_0_20px_rgba(251,191,36,0.2)] flex items-center gap-2 transform hover:scale-105 transition-transform cursor-help z-20">
              ...
              <span className="text-xs font-black text-primary uppercase tracking-widest font-league">
                Master Maker
              </span>
            </div>
          )}
            ...
            <div className="max-w-[70%]">
              ...
              <div className="flex items-center gap-3">
                ...
                {venue.facebook && (
                  <a
                    href={
                      venue.facebook.startsWith("http")
                        ? venue.facebook
                        : `https://${venue.facebook}`
                    }
                    target="_blank"
                    rel="noopener noreferrer"
      ...
      <div className="p-6 space-y-8">
          ...
          <div className="bg-red-500/10 border-2 border-red-500/50 rounded-2xl p-5 flex gap-5 animate-in zoom-in-95 duration-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start mb-1">
                <p className="text-[10px] font-black text-red-400 uppercase tracking-[0.3em] italic">
                  Live Flash Bounty
                </p>
                {venue.activeFlashBounty?.endTime && (
                  <span className="text-[9px] font-mono text-red-500 bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20">
                    ENDS{" "}
                    {formatToAMPM(
                      new Date(venue.activeFlashBounty.endTime)
                        .toTimeString()
                        .slice(0, 5),
                    )}
                  </span>
              ...
              <p className="text-sm font-bold text-red-100/80 leading-snug">
                {venue.activeFlashBounty?.description ||
                  "Limited time offer! Get down here now to claim your bonus."}
              </p>
            </div>
          </div>
        )}
        ...
        {venue.hasGameVibeCheckEnabled &&
          ...
          Object.keys(venue.liveGameStatus).length > 0 && (
            <div className="bg-slate-900/50 border border-white/5 rounded-xl p-4 animate-in fade-in slide-in-from-bottom-2 duration-500 shadow-lg">
              <div className="flex items-center gap-2 mb-3 border-b border-white/5 pb-2">
                ...
                <h4 className="text-xs font-black text-slate-300 uppercase tracking-[0.15em]">
                  Live Game Status
                </h4>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {venue.gameFeatures
                  ?.filter((f) => venue.liveGameStatus?.[f.id])
                  .map((feature) => {
                    ...
                    // Check expiration
                    const isExpired =
                      statusData.expiresAt && now > statusData.expiresAt;
                    const isTaken = statusData.status === "taken" && !isExpired;

                    const minsAgo = Math.floor(
                      (now - statusData.timestamp) / 60000,
                    );
                    const minsLeft = statusData.expiresAt
                      ? Math.ceil((statusData.expiresAt - now) / 60000)
                      : 0;

                    return (
                      <div
                        key={feature.id}
                        className="flex justify-between items-center bg-black/40 p-2 rounded-lg border border-white/5"
                      >
                        ...
                        <div className="text-right">
                          <span
                            className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${isTaken ? "bg-red-500/20 text-red-400" : "bg-green-500/20 text-green-400"}`}
                          >
                            {isTaken ? "In Use" : "Open"}
                          </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          {/* Food Service Status */}
          <div className="bg-surface border border-white/5 p-4 rounded-2xl flex flex-col items-center gap-1 shadow-lg">
            {venue.foodService === "full_kitchen" ? (
              <Utensils className="w-5 h-5 text-green-400" />
            ) : venue.foodService === "limited_kitchen" ? (
              <ChefHat className="w-5 h-5 text-yellow-400" />
            ) : venue.foodService === "snacks" ? (
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {[
            ...
            const val = (venue as any)[feature.key];
            if (!val) return null;
            ...
            const baseClass = `px-3 py-1.5 rounded-lg flex items-center gap-2 border ${colorClasses[feature.color] || colorClasses.green}`;

            if (feature.isVerified) {
              return (
                <div
                  key={feature.key}
                  className="bg-blue-500/10 border border-blue-500/30 px-4 py-2 rounded-2xl flex items-center gap-3 shadow-[0_0_15px_rgba(59,130,246,0.1)] group hover:border-blue-500/50 transition-all"
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[11px] font-bold text-blue-100 uppercase tracking-tighter">
                      {feature.label}
                    </span>
                  </div>
                </div>
            ...
            return (
              <div key={feature.key} className={baseClass}>
          ...
          {/* 2. Play Tags (Game Features) */}
          {venue.gameFeatures
            ?.filter((gf) => gf.status === "active")
            .map((game, idx) => (
              <div
                key={`game-${idx}`}
                className="bg-pink-900/20 border border-pink-500/30 px-3 py-1.5 rounded-lg flex items-center gap-2"
              >
          ...
          {/* 3. Google Services (ReadOnly) */}
          {venue.services?.map((service, idx) => (
            <div
              key={`svc-${idx}`}
              className="bg-slate-800/50 border border-slate-700 px-3 py-1.5 rounded-lg flex items-center gap-2"
            >
        ...
        {/* [NEW] Fulfillment & Ordering */}
        {venue.fulfillment &&
          Object.values(venue.fulfillment).some((v) => !!v) && (
            <div className="space-y-4">
              <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] font-league italic">
                Order & Reserve
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {venue.fulfillment.doordash && (
                  <a
                    href={venue.fulfillment.doordash}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-[#FF3008]/10 border border-[#FF3008]/20 p-4 rounded-2xl flex items-center justify-between group hover:border-[#FF3008]/50 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-[#FF3008] rounded-xl shadow-lg shadow-[#FF3008]/20 group-hover:scale-110 transition-transform">
                        <ShoppingBag className="w-4 h-4 text-white" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[9px] font-black text-[#FF3008] uppercase tracking-widest leading-none mb-1">
                          Delivery
                        </span>
                ...
                {venue.fulfillment.toast && (
                  <a
                    href={venue.fulfillment.toast}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-[#FF5C35]/10 border border-[#FF5C35]/20 p-4 rounded-2xl flex items-center justify-between group hover:border-[#FF5C35]/50 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-[#FF5C35] rounded-xl shadow-lg shadow-[#FF5C35]/20 group-hover:scale-110 transition-transform">
                        <Zap className="w-4 h-4 text-white" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[9px] font-black text-[#FF5C35] uppercase tracking-widest leading-none mb-1">
                          Direct Order
                        </span>
                        <span className="text-xs font-black text-white uppercase tracking-tight">
                          Toast Tab
                ...
                {venue.fulfillment.opentable && (
                  <a
                    href={venue.fulfillment.opentable}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-[#DA3743]/10 border border-[#DA3743]/20 p-4 rounded-2xl flex items-center justify-between group hover:border-[#DA3743]/50 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-[#DA3743] rounded-xl shadow-lg shadow-[#DA3743]/20 group-hover:scale-110 transition-transform">
                        <Utensils className="w-4 h-4 text-white" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[9px] font-black text-[#DA3743] uppercase tracking-widest leading-none mb-1">
                          Reserve
                        </span>
                        <span className="text-xs font-black text-white uppercase tracking-tight">
                          OpenTable
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {upcomingEvents.slice(0, 3).map((event) => (
                <div
                  key={event.id}
                  className="bg-slate-900/40 border border-white/5 rounded-2xl p-4 flex items-center gap-4 group hover:border-primary/20 transition-all"
                >
                  <div className="bg-surface p-3 rounded-xl border border-white/5 text-center min-w-[60px]">
                    <p className="text-[8px] font-black text-primary uppercase leading-none">
        ...
        {venue.physicalRoom !== false ? (
          ...
          <div className="bg-blue-900/20 border-2 border-blue-500/30 rounded-3xl p-6 text-center shadow-2xl relative overflow-hidden">
            ...
            <h4 className="text-lg font-black text-white uppercase font-league mb-2 tracking-tight">
              Production Only Maker
            </h4>
            <p className="text-xs text-blue-300/80 mb-4 font-bold uppercase tracking-widest leading-relaxed">
              This maker doesn't have a taproom yet. Join the scavenger hunt to
              find their brews in the wild!
            </p>
          </div>
        )}
          ...
          {venue.directMenuUrl && (
            <div className="animate-in slide-in-from-bottom-4 duration-700">
              <a
                ...
                onClick={(e) => {
                  const hasSips =
                    userProfile.favoriteDrinks?.length ||
        ...
        <div className="space-y-4">
          ...
          {activeHappyHour && (
            <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 flex gap-4 animate-in zoom-in-95 duration-300">
              ...
              <div>
                ...
                <h4 className="text-md font-black text-white uppercase font-league leading-none mb-1">
                  {activeHappyHour.specials || "Happy Hour Specials"}
                </h4>
                <p className="text-xs font-bold text-slate-300 leading-tight">
                  {activeHappyHour.description}
                </p>
                <p className="text-[9px] text-primary/60 font-black uppercase mt-2">
                  Ends at {formatToAMPM(activeHappyHour.endTime)}
                </p>
              </div>
            </div>
          )}
          ...
          {venue.leagueEvent && (
            <div className="bg-slate-900 border border-white/10 rounded-2xl p-5 space-y-4 relative overflow-hidden group shadow-2xl">
              ...
              <div className="flex gap-4 relative z-10">
                ...
                <div className="flex-1">
                  <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-1 italic">
                    League Integration
                  </p>
                  <h4 className="text-xl font-black text-white uppercase font-league tracking-wide leading-none mb-1">
                    {venue.leagueEvent === "trivia"
                      ? "Artesian Pub Quiz"
                      : `${venue.leagueEvent} Tonight`}
                  </h4>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                    Starts at {formatToAMPM(venue.triviaTime) || "7:00 PM"} •
                    Double League Points
                  </p>
                </div>
              </div>
                    ...
                    {venue.triviaHost && (
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                          Hosted By:{" "}
                          <span className="text-white">{venue.triviaHost}</span>
                        </span>
                      </div>
                    )}
                    {venue.triviaPrizes && (
                ...
                <div className="space-y-2 relative z-10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {venue.triviaHowItWorks.map((step, idx) => (
                      <div
                        key={idx}
                        className="bg-white/5 px-2 py-1 rounded text-[9px] font-bold text-slate-400 border border-white/5"
                      >
                        {idx + 1}. {step}
                      </div>
                    ))}
              ...
              <div className="space-y-3">
                {venue.happyHourRules.map((rule) => {
                  ...
                  return (
                    <div
                      key={rule.id}
                      className={`p-4 rounded-xl border transition-all ${isToday ? "bg-primary/5 border-primary/30 shadow-[0_0_15px_rgba(251,191,36,0.05)]" : "bg-black/20 border-white/5 opacity-60"}`}
                    >
                      <div className="flex justify-between items-start mb-1">
                        <div className="flex flex-wrap gap-1">
                          {rule.days.map((d) => (
                            <span
                              key={d}
                              className={`text-[8px] font-black px-1.5 py-0.5 rounded ${isToday && d === currentDay ? "bg-primary text-black" : "bg-white/10 text-slate-400"}`}
                            >
                              {d}
                            </span>
                          ))}
                        ...
                        <span className="text-[10px] font-mono text-primary font-bold">
                          {formatToAMPM(rule.startTime)} -{" "}
                          {formatToAMPM(rule.endTime)}
                        </span>
                      </div>
                      <h5 className="text-sm font-black text-white uppercase font-league tracking-wide">
                        {rule.specials}
                      </h5>
          ...
          {venue.weekly_schedule &&
            Object.keys(venue.weekly_schedule).length > 0 && (
              <div className="space-y-4">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] font-league italic">
                  Weekly Rituals
                </h3>
                <div className="bg-surface border border-white/5 rounded-2xl overflow-hidden shadow-xl">
                  {(
                    ...
                    return (
                      <div
                        key={day}
                        className={`flex items-center justify-between p-4 border-b border-white/5 last:border-0 ${isToday ? "bg-primary/10" : ""}`}
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-1 h-1 rounded-full ${isToday ? "bg-primary animate-pulse" : "bg-slate-700"}`}
                          />
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest ${isToday ? "text-primary" : "text-slate-500"}`}
                          >
                            {day}
                          </span>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                          {activities.map((act, i) => (
                            <span
                              key={i}
                              className="text-xs font-black text-white uppercase font-league tracking-wide"
                            >
                              {act}
                            </span>
                          ))}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {venue.gameFeatures.map((feature) => {
                  ...
                  const config = ASSET_MAP[feature.type] || {
                    icon: LayoutGrid,
                    label: feature.name,
                  };
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-hidden">
                        <p
                          className={`text-[10px] font-black uppercase truncate ${isOutOfOrder ? "text-red-400 line-through" : "text-white"}`}
                        >
              ...
              <div className="grid grid-cols-1 gap-2">
                {venues
                  ...
                  .map((carrier) => (
                    <div
                      key={carrier.id}
                      onClick={() => navigate(`/bars/${carrier.id}`)}
                      className="flex items-center justify-between p-3 bg-black/40 rounded-lg border border-white/5 cursor-pointer hover:bg-white/5 transition-colors group"
                    >
                      <div className="flex items-center gap-3">
        ...
        {venue.physicalRoom !== false && (
          <div className="space-y-4">
            ...
            <div
              className="bg-surface border border-white/5 rounded-2xl p-4 flex justify-between items-center transition-all hover:border-primary/30 cursor-pointer"
              onClick={() =>
                venue.address &&
                window.open(
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-bold text-white uppercase">
                    {4 - flightItems.length} slots remaining
                  </span>
                </div>
              </div>
              <button
            ...
            <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
              <div className="bg-slate-900 border border-white/10 w-full max-w-sm rounded-3xl overflow-hidden relative shadow-2xl">
                ...
                <div className="p-8 text-center bg-slate-800/50">
                  ...
                  <h3 className="text-2xl font-black text-white uppercase font-league tracking-tighter mb-1">
                    Your Flight Deck
                  </h3>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-black/40 p-4 rounded-2xl border border-white/5 text-center">
                      ...
                      <p
                        className={`text-3xl font-black font-league ${parseFloat(flightStats.abv as string) > 8.0 ? "text-red-500" : "text-white"}`}
                      >
                        {flightStats.abv}%
                      </p>
                    </div>
                    <div className="bg-black/40 p-4 rounded-2xl border border-white/5 text-center">
                      ...
                      <p className="text-3xl font-black text-white font-league">
                        $$
                      </p>
                    </div>
                  </div>
                  ...
                  {parseFloat(flightStats.abv as string) > 8.0 && (
                    <div className="bg-red-900/20 border border-red-500/30 p-4 rounded-xl flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-black text-red-400 uppercase tracking-wide mb-1">
                          High Gravity Alert
                        </p>
                        <p className="text-[10px] text-red-300 leading-relaxed">
                          This flight packs a punch. Please drink responsibly
                          and arrange a safe ride home.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <button
                    onClick={() => {
                      if (!userProfile) {
                        showToast(
                          "Please log in to save your flight!",
                          "error",
                        );
                      ...
                      showToast(
                        "Flight Saved to Profile! (Coming Soon: View Flights)",
                        "success",
                      );

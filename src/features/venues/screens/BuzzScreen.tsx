    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-400 line-clamp-2 leading-relaxed">
          {event.description ||
            `Join us at ${event.venue.name} for ${event.title}!`}
        </p>
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase truncate max-w-[150px]">
              {event.venue.address || "Olympia, WA"}
            </span>
          </div>
          <ChevronRight
            size={14}
            className="text-primary opacity-0 group-hover:opacity-100 transition-all translate-x-1"
          />
        </div>
      </div>
    </div>
...
export const BuzzScreen: React.FC = () => {
  ...
  // [DEALS FEED] Subscribe to upcoming deals (Next 7 Days)
  React.useEffect(() => {
    const now = Date.now();
    // Query: endTime > now (Standard validity check)
    // We filter the "Too far in future" deals client-side if needed, but registry is small.
    const q = query(
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach((doc) => {
        const data = doc.data() as ScheduledDeal;
        // Client-side safety: Only show deals starting within 7 days
        if (data.startTime < now + 7 * 24 * 60 * 60 * 1000) {
          if (!grouped[data.venueId || ""]) grouped[data.venueId || ""] = [];
          grouped[data.venueId || ""].push(data);
        }
      });
      setUpcomingDeals(grouped);
    });
    ...
    (v: Venue): boolean => {
      ...
      if (searchQuery) {
        ...
        const nameMatch =
          ...
          (v.nicknames?.some((n) => n.toLowerCase().includes(q)) ?? false);

        // 2. Events (Weekly, Special, League)
        const eventTypeMatch =
          v.leagueEvent?.toLowerCase().includes(q) ||
          (v.special_events?.some(
            (e) =>
        ...
        const scheduleMatch =
          v.weekly_schedule &&
          Object.values(v.weekly_schedule)
            .flat()
            .some((ev) => (ev as string).toLowerCase().includes(q));
        const eventMatch = eventTypeMatch || scheduleMatch;

        // 3. Games & Services
        const gameMatch =
          v.gameFeatures?.some((f) =>
            (f.name?.toLowerCase() || f.type?.toLowerCase() || "").includes(q),
          ) ?? false;
        const serviceMatch =
          v.services?.some((s) => s.toLowerCase().includes(q)) ?? false;
        const playMatch = gameMatch || serviceMatch;
      ...
      if (!isToday) {
        ...
        const hasEvents =
          (v.weekly_schedule?.[dayNameFull.toLowerCase()]?.length ?? 0) > 0 ||
          (v.weekly_schedule?.[dayNameShort]?.length ?? 0) > 0 ||
          (v.special_events?.some((e) =>
        ...
        const isOpenOnDay =
          ...
          (hours[dayNameFull] ||
            hours[dayNameShort] ||
            hours[dayNameFull.toLowerCase()] ||
            hours[dayNameShort.toLowerCase()]);
      ...
      if (filterKind === "deals") {
        ...
        // 1.5 Upcoming Flash Bounty (Starting Soon)
        if (upcomingDeals[v.id]?.length > 0) return true;
      ...
      if (filterKind === "features" && featureFilter !== "all") {
        ...
        if (q === "all_ages")
          return v.isAllAges === true || v.attributes?.minors_allowed === true;
        if (q === "dog_friendly") return v.isDogFriendly === true;
        if (q === "patio") return v.hasOutdoorSeating === true;
        return v.services?.some((s) => s.toLowerCase().includes(q)) ?? false;
      }
      ...
      if (filterKind === "events") {
        ...
        if (eventFilter === "other") {
          const hasAnyEvent =
            (v.special_events?.length ?? 0) > 0 ||
            (v.weekly_schedule && Object.keys(v.weekly_schedule).length > 0) ||
            !!v.leagueEvent;

          const isMainEvent = TAXONOMY_EVENTS.some(
            (main) =>
              v.leagueEvent?.toLowerCase().includes(main.toLowerCase()) ||
              v.special_events?.some(
                (e) =>
                  e.title.toLowerCase().includes(main.toLowerCase()) ||
                  e.type.toLowerCase().includes(main.toLowerCase()),
              ) ||
              (v.weekly_schedule &&
                Object.values(v.weekly_schedule)
                  .flat()
                  .some((ev) =>
                    (ev as string).toLowerCase().includes(main.toLowerCase()),
                  )),
          );
      ...
      if (v.tier_config?.is_directory_listed === false || v.isActive === false)
        return false;
      ...
      if (isNextStopMode) {
        ...
        const windowMs =
          (PULSE_CONFIG.WINDOWS.LCB_WINDOW || 12) * 60 * 60 * 1000;
        const twelveHoursAgo = Date.now() - windowMs;

        const wasClockedInRecently = clockInHistory.some(
          (h) => h.venueId === v.id && h.timestamp > twelveHoursAgo,
        );
        const wasVibeCheckedRecently = vibeCheckHistory.some(
          (h) => h.venueId === v.id && h.timestamp > twelveHoursAgo,
        );
  ...
  const filteredVenues = React.useMemo(
    () =>
      [...venuesWithDistance].filter(applyFilter).sort((a, b) => {
        ...
        if (filterKind === "deals") {
          ...
          const getSortMetric = (v: Venue) => {
            ...
            // Priority 2.5: Upcoming Flash Bounty (Starting Soonest)
            // Score: 3000 + Minutes until start
            // This places it AFTER Active Flash (-3000) and Active HH (0-1440),
            // but BEFORE Upcoming HH (5000+)
            const nextDeal = upcomingDeals[v.id]?.sort(
              (d1, d2) => d1.startTime - d2.startTime,
            )[0];
            ...
            if (nextDeal) {
              ...
              return 3000 + Math.max(0, diffMinutes);
            }
            ...
            const upcomingRule = rules
              ...
              .sort(
                (r1, r2) =>
                  timeToMinutes(r1.startTime) - timeToMinutes(r2.startTime),
              )[0];
        ...
        if (filterKind === "events") {
          const getEventScore = (v: Venue) => {
            ...
            const minSpecial =
              specialHours.length > 0 ? Math.min(...specialHours) : 2000;
        ...
        if (a.hourStatus !== "open" && b.hourStatus === "open") return 1;
        if (a.hourStatus === "last_call" && b.hourStatus === "closed")
          return -1;
  ...
  const displayItems = useMemo(() => {
    ...
    const dayNameShort = dayNameFull.substring(0, 3);
    const dayNameShortCap =
      dayNameFull.charAt(0).toUpperCase() + dayNameFull.slice(1, 3);
    ...
    return displayVenues
      .flatMap((v) => {
        ...
        // 2. Weekly Schedule
        const weekly =
          v.weekly_schedule?.[dayNameFull] ||
          v.weekly_schedule?.[dayNameShortCap] ||
          v.weekly_schedule?.[dayNameShort] ||
          [];
        weekly.forEach((title) => {
          const matchesEventFilter =
            eventFilter === "all" ||
            title.toLowerCase().includes(eventFilter.toLowerCase());
          if (matchesEventFilter || eventFilter === "other") {
            items.push({
              title,
              isEvent: true,
    ...
    <div className="bg-background min-h-screen pb-24 font-sans text-slate-100">
      <SEO
        jsonLd={{
          ...
          description:
            "The Nightlife Operating System for Thurston County, WA.",
      ...
      <h1 className="sr-only">
        OlyBars Pulse: Real-Time Thurston County Nightlife Vibe Map
      </h1>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs font-bold text-white">
                      Join the Artesian Bar League
                    </p>
                  </div>
                </div>
                <ChevronRight
                  size={16}
                  className="text-primary group-hover/cta:translate-x-1 transition-transform"
                />
              </button>
            </div>
          )}
              ...
              {isLoading ? (
                ...
                <div className="text-center py-20 bg-surface/30 rounded-3xl border-2 border-dashed border-white/5 mx-4">
                  ...
                  <p className="text-slate-500 font-bold uppercase tracking-widest italic">
                    {isNextStopMode
                      ? "Run Complete! You've cleared the list."
                      : "No Spots Found"}
                  </p>
                  {isNextStopMode ? (
                    <button

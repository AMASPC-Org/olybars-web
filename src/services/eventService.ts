...
import { VenueOpsService } from "./VenueOpsService";
...
export class EventService {
  ...
  static async submitEvent(
    event: Omit<AppEvent, "id" | "status" | "createdAt" | "submittedBy">,
    ...
    try {
      ...
      // Notify SuperAdmin
      try {
        await VenueOpsService.notifySuperAdmin({
          title: "New Event Submitted",
          message: `A new event has been submitted: ${event.title}`,
          type: "ACTION_LOG",
          priority: 2,
          payload: {
            title: event.title,
  ...
  ): Promise<import("../types").EventAnalysis> {
    const { API_BASE_URL } = await import("../lib/api-config");
    try {

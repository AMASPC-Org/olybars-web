...
if (!PROJECT_ID) {
    console.error('❌ Error: GOOGLE_CLOUD_PROJECT environment variable or project ID argument is required.');
    process.exit(1);
}
...
function runKeywordScan() {
    ...
    SCAN_DIRECTORIES.forEach(dir => {
        ...
        files.forEach(file => {
            ...
            lines.forEach((line, index) => {
                ...
                if (line.includes('@guardrail-ignore') || line.includes('COMPLIANCE') || line.includes('LCB_FORBIDDEN')) return;

                FORBIDDEN_KEYWORDS.forEach(word => {
                    const regex = new RegExp(`\\b${word}\\b`, 'gi');
                    if (regex.test(line)) {
                        console.error(`❌ [VIOLATION] Forbidden keyword "${word}" found in: ${path.relative(rootDir, file)}:${index + 1}`);
                        violationCount++;
                    }
                });
            });
    ...
    if (violationCount > 0) {
        console.error(`🛑 [FAILED] WSLCB Scan failed with ${violationCount} violations.`);
        return false;
    }
...
function runMapsAudit() {
    ...
    try {
        const output = execSync(`gcloud alpha services api-keys list --project=${PROJECT_ID} --format="json"`, { encoding: 'utf8' });
        const keys = JSON.parse(output);

        let insecureKeys = 0;
        keys.forEach((key: any) => {
            const displayName = key.displayName || 'Unnamed Key';
            const isMapsKey = displayName.toLowerCase().includes('maps') || displayName.toLowerCase().includes('browser');
            ...
            if (isMapsKey) {
                ...
                if (allowedReferrers.length === 0) {
                    console.error(`❌ [SECURITY] Maps API Key "${displayName}" has NO HTTP Referrer restrictions!`);
                    insecureKeys++;
                } else {
                    console.log(`✅ [SECURE] Maps API Key "${displayName}" is restricted.`);
                }
            }
        });

        if (insecureKeys > 0) {
            console.error('🛑 [FAILED] Insecure Maps API Keys detected in cloud configuration.');
            return false;
        }
        return true;
    } catch (error: any) {
        console.warn('⚠️ [WARN] Could not audit API keys. Ensure apikeys.googleapis.com is enabled.');
        return true;
    }
}
...
function runIAMCheck() {
    ...
    try {
        ...
        console.log(`🔍 Checking permissions for: ${currentAccount} (Type: ${isServiceAccount ? 'SA' : 'User'})`);
        ...
        const policyOutput = execSync(`gcloud projects get-iam-policy ${PROJECT_ID} --format="json"`, { encoding: 'utf8' });
        ...
        policy.bindings.forEach((binding: any) => {
            if (elevatedRoles.includes(binding.role)) {
                if (binding.members.includes(`serviceAccount:${currentAccount}`)) {
                    console.error(`❌ [SECURITY_DRIFT] Service Account ${currentAccount} has elevated role: ${binding.role}`);
        ...
        if (hasPrivilegeDrift) {
            console.error('🛑 [FAILED] IAM Sentinel detected Privilege Drift. Build halted for security.');
            return false;
        }
        console.log('✅ [PASS] IAM Sentinel confirmed Least Privilege.');
        return true;
    } catch (error: any) {
        console.warn('⚠️ [WARN] IAM Sentinel could not fetch policy. Ensure iam.securityReviewer role is granted.');
        return true;
    }
}

    ...
    try {
        ...
        if (output) {
            ...
            for (const pid of pids) {
                try {
                    execSync(IS_WINDOWS ? `taskkill /F /PID ${pid}` : `kill -9 ${pid}`, { stdio: 'ignore' });
                } catch (e) { }
            }
        }
    } catch (e) { }
...
async function heal() {
    ...
    startService('emulators', IS_WINDOWS ? 'npx.cmd firebase emulators:start' : 'npx firebase emulators:start');
    ...
    try {
        ...
        console.warn('⚠️ Seed warning - if this is a fresh start, emulators might still be booting.');
    }

...
## 4. The "Drunk Thumb" Reality

We build for dark bars and distracted users.

- **Touch Targets**: Minimum **44x44px** for all interactive elements (Buttons, Tab Bars, List Items).
- **Padding Sanity**: Important actions (e.g., "Clock In") must be placed in the "Thumb Zone" (bottom half of screen). Ensure interactive areas have enough "Breathing Room" to prevent accidental clicks.

## 5. Aesthetics vs. Logic

- **Visual Evidence**: If the logic says a bar is "Open", but the UI shows "Closed", the UI is broken regardless of the database state.
- **Contrast Check**: Never compromise legibility for style. Branded backgrounds must use high-contrast text overlays (Ref: Section 3.2).

...
const stripSensitiveVenueData = (venue: Venue, brief = false): Venue => {
  ...
  const sensitiveFields: (keyof Venue)[] = [
    "partnerConfig",
    "pointBank",
    "pointBankLastReset",
    "wifiPassword",
...
export const updateVenueBuzz = async (venueId: string) => {
  ...
  signals.forEach((data) => {
    ...
    if (
      data.timestamp > liveWindowAgo &&
      (data.type === "clock_in" || data.type === "vibe_report")
    ) {
  ...
  const finalStatus =
    venueData?.manualStatus && venueData?.manualStatusExpiresAt > now
  ...
  if (isConsensusPacked && !wasConsensusPacked) {
    ...
    await NotificationService.dispatchPulseAlert(
      venueId,
      venueData?.name || "A venue",
    );
...
const applyVirtualDecay = (venue: Venue): Venue => {
  ...
  if (venue.currentBuzz?.score && venue.currentBuzz.lastUpdated) {
    decayedScore = calculateDecayedScore(
      venue.currentBuzz.score,
      venue.currentBuzz.lastUpdated,
      now,
    );
  }
  ...
  if (
    !(
      venue.manualStatus &&
      venue.manualStatusExpiresAt &&
      venue.manualStatusExpiresAt > now
    )
  ) {
  ...
  if (
    venue.manualClockIns !== undefined &&
    venue.manualClockInsExpiresAt &&
    venue.manualClockInsExpiresAt > now
  ) {
...
export const getVenueById = async (venueId: string): Promise<Venue | null> => {
  ...
  if (venueCache) {
    ...
    if (cached)
      return applyVirtualDecay(stripSensitiveVenueData(cached, false));
  }
...
export const fetchVenues = async (brief = false): Promise<Venue[]> => {
  ...
  if (venueCache) {
    ...
    if (isStale) {
      // Background refresh
      refreshVenueCache().catch((err) =>
        console.error("[Backend] Background cache update failed:", err),
      );
    }
    return venueCache.data
      .map(applyVirtualDecay)
      .map((v) => stripSensitiveVenueData(v, brief));
  }
...
export const clockIn = async (
  ...
  const signal: Partial<Signal> = {
    ...
    type: "clock_in",
    timestamp,
    expiresAt: expiresAt as any, // Cast for Firestore Timestamp or Date
    verificationMethod,
  };
...
export const performVibeCheck = async (
  ...
  const expiresAt = new Date(now + 7 * 24 * 60 * 60 * 1000); // [FINOPS] 7-day TTL for vibe reports
  ...
  if (soberFriendlyCheck && venueData.isSoberFriendly) {
    ...
    if (isGPS && !recentClockIn.empty) {
      ...
      const userRecentReport = reports.find(
        (r) => r.userId === userId && r.timestamp > thirtyDaysAgo,
      );
      ...
      if (!userRecentReport) {
        ...
        const newReport = {
          userId,
          timestamp: now,
          reason: soberFriendlyCheck.isGood
            ? undefined
            : soberFriendlyCheck.reason,
        };
        ...
        if (!soberFriendlyCheck.isGood) {
          ...
          if (negativeReports.length === 1) {
            venueUpdates.soberFriendlyNote =
              "Heads up! A guest wasn't impressed with the NA options tonight. Check your stock.";
          }

          // B. Deactivation Logic
          const sevenDaysAgo = now - 7 * 24 * 60 * 60 * 1000;
          const recentNegatives = negativeReports.filter(
            (r) => r.timestamp > sevenDaysAgo,
          );
          const uniqueUserCount = new Set(recentNegatives.map((r) => r.userId))
            .size;
          ...
          if (
            uniqueUserCount >= THRESHOLD_UNIQUE_7D ||
            negativeReports.length >= THRESHOLD_TOTAL
          ) {
            ...
            console.log(
              `[SOBER_FRIENDLY] Venue ${venueId} auto-disabled. Unique(7d): ${uniqueUserCount}, Total: ${negativeReports.length}`,
            );
          }
        } else {
 ...
 */
export const clockInGameFeature = async (
  venueId: string,
  userId: string,
  featureId: string,
) => {
  ...
  // Check if the venue actually has this game feature
  const feature = venueData.gameFeatures?.find((a) => a.id === featureId);
  if (!feature) throw new Error(`Venue does not have ${featureId}`);
  ...
  return {
    success: true,
    message: `Clocked in for ${feature.name} at ${venueData.name}!`,
    pointsAwarded: 5,
  };
};
...
export const checkAndAwardBadges = async (
  ...
  for (const badge of BADGES) {
    ...
    if (badge.criteria.type === "clockin_set" && badge.criteria.venueIds) {
      ...
      const hasAll = badge.criteria.venueIds.every((vid) =>
        uniqueVenues.has(vid),
      );
      if (hasAll) unlocked = true;
    } else if (badge.criteria.type === "count" && badge.criteria.count) {
      const matchCount = (badge.criteria.venueIds || []).filter((vid) =>
        uniqueVenues.has(vid),
      ).length;
    ...
    if (
      badge.id === "the_historian" &&
      badge.criteria.isHistoricalAnchor &&
      badge.criteria.timeWindowDays
    ) {
      const historyLimit =
        Date.now() - badge.criteria.timeWindowDays * 24 * 60 * 60 * 1000;
    ...
    if (unlocked) {
      ...
      if (badge.criteria.type === "clockin_set" && badge.criteria.venueIds) {
        const visitedCount = badge.criteria.venueIds.filter((vid) =>
          uniqueVenues.has(vid),
        ).length;
        ...
        await userRef.update({
          [`badges.${badge.id}`]: {
            ...
            progress: progress,
            unlocked: false,
            completedVenueIds: badge.criteria.venueIds.filter((vid) =>
              uniqueVenues.has(vid),
            ),
          },
        });
...
export const logUserActivity = async (data: {
  ...
  if (userDoc.exists) {
    ...
    if (!isPending) {
      updates["stats.seasonPoints"] =
        (userData?.stats?.seasonPoints || 0) + data.points;
      updates["stats.competitionPoints"] =
        (userData?.stats?.competitionPoints || 0) + data.points;
    }
    ...
    await userRef.set({
      ...
      stats: {
        ...
        lifetimeClockins:
          data.type === "clock_in" || data.type === "clockin" ? 1 : 0,
        currentStreak: 0,
      },
      role: "user",
    });
...
export const updateVenue = async (
  ...
  if (requestingUserId) {
    ...
    isAdmin =
      ...
      userData?.role === "admin" ||
      userData?.email === "ryan@amaspc.com";
  ...
  const ownerManagerFields: (keyof Venue)[] = [
    ...
    "hasOutdoorSeating",
    "privateSpaces",
    "hasPrivateRoom", // [FIX] Whitelist private spaces
    "isCinderella",
    "cinderellaHours",
    "guestPolicy",
    "membershipRequired",
  ...
  Object.keys(updates).forEach((key) => {
    ...
    if (
      isAdmin &&
      (adminOnlyFields.includes(field) || ownerManagerFields.includes(field))
    ) {
  ...
  if (!isAdmin && !isOwner && !isManager) {
    ...
    const nonPlayerFields = Object.keys(filteredUpdates).filter(
      (k) => !playerFields.includes(k as any),
    );
    if (nonPlayerFields.length > 0) {
      throw new Error(
        "Unauthorized: You only have permission to update vibe and game status.",
      );
    }
  }
  ...
  if (Object.keys(filteredUpdates).length === 0) {
    throw new Error(
      "No valid update fields provided or insufficient permissions for selected fields.",
    );
  }
  ...
  if (
    ...
    filteredUpdates.address !== venueData.address
  ) {
    console.log(
      `[GEOCODE] Address changed for ${venueId}. Re-resolving coordinates...`,
    );
    const geoResult = await geocodeAddress(filteredUpdates.address);
    if (geoResult) {
      filteredUpdates.location = { lat: geoResult.lat, lng: geoResult.lng };
      console.log(
        `[GEOCODE] Successfully resolved ${filteredUpdates.address} to ${geoResult.lat}, ${geoResult.lng}`,
      );
    } else {
      console.warn(
        `[GEOCODE] Failed to resolve address: ${filteredUpdates.address}`,
      );
    }
  }
...
export const syncVenueWithGoogle = async (
  ...
  if (now - lastSynced < SYNC_COOLDOWN && !manualPlaceId) {
    const hoursRemaining = Math.ceil(
      (SYNC_COOLDOWN - (now - lastSynced)) / (60 * 60 * 1000),
    );
    console.warn(
      `[PLACES_SYNC] Throttled for ${venueData.name}. Last sync was recent. Try again in ${hoursRemaining}h.`,
    );
    return {
      success: false,
  ...
  console.log(
    `[PLACES_SYNC] Starting sync for ${venueData.name} (${venueId})...`,
  );
  ...
  if (!placeId) {
    ...
    if (searchResult) {
      ...
      console.log(
        `[PLACES_SYNC] Found placeId: ${placeId} for ${venueData.name}`,
      );
    } else {
      throw new Error(
        `Could not find a matching place on Google for "${venueData.name}".`,
      );
    }
  }
  ...
  if (!details) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        `[SYNC_BYPASS] Failed to fetch real details for ${placeId}. Using mock data for testing.`,
      );
      details = {
        place_id: placeId,
      ...
      throw new Error(
        `Failed to fetch details for Google Place ID: ${placeId}`,
      );
    }
  }

  // 3. Prepare updates
  // [SAFETY_LOCK] We iterate on existing data rather than resetting to false/empty.
  // This allows Owners to manually 'dial in' features that Google might miss, without fear of wiping.
  const updates: Partial<Venue> = {
    googlePlaceId: placeId,
    lastGoogleSync: now,
    updatedAt: now,
    // Preserve existing flags (Owner override), allowing Google to *confirm* but not *erase*
    isAllAges: venueData.isAllAges || false,
    isDogFriendly: venueData.isDogFriendly || false,
    hasOutdoorSeating: venueData.hasOutdoorSeating || false,
    isSoberFriendly: venueData.isSoberFriendly || false,
    hasPrivateRoom: venueData.hasPrivateRoom || false,
    gameFeatures: venueData.gameFeatures || [],
  };

  if (details.name && !venueData.isLocked) updates.name = details.name;
  if (details.formatted_phone_number && !venueData.isLocked)
    updates.phone = details.formatted_phone_number;
  if (details.website && !venueData.isLocked) updates.website = details.website;
  if (details.formatted_address && !venueData.isLocked)
    updates.address = details.formatted_address;

  if (venueData.isLocked) {
    console.log(
      `[PLACES_SYNC] ${venueData.name} is identity-locked. Skipping primary field sync.`,
    );
  }
  ...
  if (details.photos && details.photos.length > 0) {
    ...
    const apiKey =
      process.env.GOOGLE_BACKEND_KEY || process.env.VITE_GOOGLE_MAPS_API_KEY;
    (updates as any).googlePhotoUrl =
  ...
  // Map specific flags if present in Google Details (Only set to TRUE, never force false)
  ...
  // Preserve and Merge Game Features (No duplicates)
  // Note: Google doesn't typically return 'pool_table' etc in the main details object easily,
  // but if we were parsing types/tags, we would add them here.
  // For now, we trust the Owner's game features and just ensure we don't wipe them.
  ...
  console.log(
    `[PLACES_SYNC] Successfully synced ${venueData.name} with Google.`,
  );
...
export const onboardVenue = async (
  ...
  console.log(
    `[ONBOARDING] Starting onboarding for Google Place: ${googlePlaceId} by User: ${ownerId} (Role: ${requesterRole})`,
  );
  ...
  if (status.isClaimed) {
    console.warn(
      `[ONBOARDING] Failed: Venue ${googlePlaceId} already claimed.`,
    );
  ...
  if (!status.exists) {
    ...
    if (requesterRole !== "super-admin") {
      console.warn(
        `[SECURITY_ALERT] Unauthorized attempt to add a new bar by: ${ownerId}`,
      );
      throw new Error(
        "Only the OlyBars Super-Admin can add a new bar to the official directory. To request a listing, contact ryan@amaspc.com.",
      );
    }
    ...
    console.log(
      `[ONBOARDING] Updating existing venue ${venueId} with owner ${ownerId}`,
    );
    await db.collection("venues").doc(venueId!).update({
      ownerId,
      updatedAt: Date.now(),
...
export const addVenueMember = async (
  ...
  if (!canAdd) {
    throw new Error(
      "Unauthorized: You do not have permission to add members to this venue.",
    );
  }
  ...
  if (userSnapshot.empty) {
    throw new Error(
      `User with email ${email} not found. They must sign in to OlyBars once before being added to a team.`,
    );
  }
...
export const removeVenueMember = async (
  ...
  if (!canRemove) {
    throw new Error(
      "Unauthorized: You do not have permission to remove members from this venue.",
    );
  }
  ...
  const managerIds = (venueData.managerIds || []).filter(
    (id) => id !== memberId,
  );
  await venueRef.update({ managerIds });
...
export const getVenueMembers = async (venueId: string) => {
  ...
  const snapshot = await usersRef
    .where(`venuePermissions.${venueId}`, "in", ["owner", "manager", "staff"])
    .get();
...
export const generateVenueInsights = async (venueId: string) => {
  ...
  return await gemini.generateManagerSuggestion(stats, {
    id: venueId,
    ...venueDoc.data(),
  });
};
 ...
 */
export const getPartnerHourlyReport = async (
  venueId: string,
  dayTimestamp?: number,
) => {
  ...
  const hourlyData: Record<
    number,
    { clockins: number; vibeReports: number; points: number }
  > = {};
  for (let i = 0; i < 24; i++) {
 ...
 */
export const reviewBounty = async (
  submissionId: string,
  ...
  if (!logsSnapshot.empty) {
    ...
    if (status === "APPROVED") {
      ...
      if (userDoc.exists) {
        await userRef.update({
          "stats.seasonPoints":
            (userData?.stats?.seasonPoints || 0) +
            submissionData.pointsPotential,
          "stats.competitionPoints":
            (userData?.stats?.competitionPoints || 0) +
            submissionData.pointsPotential,
        });
      }
    }
...
export const fetchAllVenuesAdmin = async (): Promise<Venue[]> => {
  ...
  try {
    ...
    const venues = snapshot.docs.map(
      (doc) => ({ id: doc.id, ...doc.data() }) as Venue,
    );
    // Sort alphabetically
